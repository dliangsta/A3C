...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while j < 99:
            ...
            Q1 = sess.run(Qout, feed_dict={inputs1: np.identity(
                env.observation_space.n)[s1:s1 + 1]})
            # Obtain maxQ' and set our target value for chosen action.
            maxQ1 = np.max(Q1)
            targetQ = allQ
            targetQ[0, a[0]] = r + y * maxQ1
            # Train our network using target and predicted Q values
            _, W1 = sess.run([updateModel, W], feed_dict={inputs1: np.identity(
                env.observation_space.n)[s:s + 1], nextQ: targetQ})
            rAll += r
            s = s1
            if d == True:
        ...
        jList.append(j)
        rList.append(rAll)
        print("Avg score: " + str(sum(rList) / (j + 1)))
